...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	*/
		...
		onBattleStart() {
			...
			onModifyMove(move) {
				if (move.secondaries) {
					this.debug('nullifying secondary chance');
					for (const secondary of move.secondaries) {
						if (secondary.chance && secondary.chance < 50) secondary.chance = 0;
					}
				}
				if (move.self?.chance && move.self.chance < 50) move.self.chance = 0;
			},
	},
};
